...
void quantize_vector(type* v, type* vplus, type* vminus, int x, int D) {
    ...
    int i = 0;
    __m256d sign_mask = _mm256_set1_pd(-0.0);  // maschera per azzerare bit segno
...
// Versione AVX baseline per approx_distance
...
// Versione AVX baseline per euclidean_distance
